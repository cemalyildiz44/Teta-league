...
import { PLATFORM_OPTIONS, POSITION_FILTER_OPTIONS, getPositionLabel } from '@/app/utils/positions';
...
export default function ClientAccount({ profile, authUser, team, league, isCaptain, achievements, pendingInvites }: any) {
  ...
  // Position States
  const [selectedAltPositions, setSelectedAltPositions] = useState<string[]>(() => {
    if (Array.isArray(profile?.alternative_positions)) {
      return profile.alternative_positions.filter(Boolean);
    }
    return [];
  });
  const [primaryPos, setPrimaryPos] = useState<string>(profile?.primary_position || 'Belirtilmedi');
  ...
  const handleProfileSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Yan pozisyonları JSON array formatında ayarla
    formData.set('alternative_positions', JSON.stringify(selectedAltPositions));
      ...
      <section className="bg-[#03070c] border border-white/5 rounded-2xl p-6 lg:p-8 flex flex-col md:flex-row items-center md:items-start gap-6">
        ...
        <div className="flex flex-col flex-1 text-center md:text-left min-w-0">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-4">
            ...
            <span className="px-2.5 py-0.5 bg-[#00e5ff]/10 border border-[#00e5ff]/20 text-[10px] font-[900] tracking-widest text-[#00e5ff] uppercase rounded">
              {getPositionLabel(profile?.primary_position) || 'BELİRTİLMEDİ'}
            </span>
            {profile?.alternative_positions && profile.alternative_positions.length > 0 && (
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                {profile.alternative_positions.map((pos: string) => (
                  <span
                    key={pos}
                    className="px-2.5 py-0.5 bg-white/5 border border-white/10 text-[10px] font-[700] tracking-wide text-gray-300 uppercase rounded"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setPrimaryPos(val);
                      if (selectedAltPositions.includes(val)) {
                        setSelectedAltPositions(prev => prev.filter(p => p !== val));
                      }
                    }}
                    className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-[14px] font-[700] text-white focus:outline-none focus:border-[#00e5ff]/50 transition-colors appearance-none cursor-pointer"
                  >
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="block text-[10px] font-[900] text-gray-500 tracking-widest uppercase">
                      YAN POZİSYONLAR ({selectedAltPositions.length}/4)
                    </label>
                    {selectedAltPositions.length > 0 && (
                      <button
                        type="button"
                        onClick={() => setSelectedAltPositions([])}
                        className="text-[10px] font-[700] text-gray-500 hover:text-red-400 transition-colors uppercase tracking-wider"
                      >
                        Temizle
                      </button>
                    )}
                  ...
                  <div className="bg-black/40 border border-white/10 rounded-xl p-2.5 min-h-[48px] flex flex-col gap-2">
                    {/* Selected Position Chips */}
                    {selectedAltPositions.length > 0 && (
                      <div className="flex flex-wrap items-center gap-1.5">
                        {selectedAltPositions.map((pos) => {
                          const opt = POSITION_FILTER_OPTIONS.find(p => p.value === pos);
                          return (
                            <span
                              key={pos}
                              className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-white/5 border border-white/10 text-gray-200 rounded-lg text-[11px] font-[700] tracking-wide"
                            >
                              <span>{opt ? opt.label : pos}</span>
                              <button
                                type="button"
                                onClick={() => setSelectedAltPositions(prev => prev.filter(p => p !== pos))}
                                className="w-3.5 h-3.5 rounded-full flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-colors text-xs leading-none"
                                title="Kaldır"
                              >
                                &times;
                              </button>
                    ...
                    {/* Add Select Dropdown */}
                    {selectedAltPositions.length < 4 ? (
                      <div className="relative">
                        <select
                          value=""
                          onChange={(e) => {
                            const pos = e.target.value;
                            if (!pos || selectedAltPositions.length >= 4 || pos === primaryPos || selectedAltPositions.includes(pos)) return;
                            setSelectedAltPositions(prev => [...prev, pos]);
                            e.target.value = '';
                          }}
                          className="w-full bg-[#03070c] border border-white/10 hover:border-white/20 rounded-lg px-3 py-2 text-[12px] font-[600] text-gray-300 focus:outline-none focus:border-[#00e5ff]/50 transition-colors appearance-none cursor-pointer"
                        >
                          <option value="" disabled>
                            {selectedAltPositions.length === 0 ? '+ Yan Pozisyon Ekle...' : `+ Yan Pozisyon Ekle (${4 - selectedAltPositions.length} hak kaldı)...`}
                          </option>
                          {POSITION_FILTER_OPTIONS.filter(p => p.value !== primaryPos && !selectedAltPositions.includes(p.value)).map(pos => (
                            <option key={pos.value} value={pos.value} className="bg-[#03070c] text-white">
                              {pos.label}
                            </option>
                          ))}
                        </select>
                      ...
                      <div className="text-[11px] font-semibold text-gray-500 italic py-0.5 px-1">
                        Maksimum 4 yan pozisyon seçildi.
                      </div>
                    )}
                  </div>
                </div>
