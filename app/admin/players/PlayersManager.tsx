...
export function PlayersManager({ players, teams }: PlayersManagerProps) {
  ...
  const getAchievementBadgeColor = (type: string) => {
    switch (type) {
      ...
      case 'TOTS': return 'bg-cyan-500/20 text-cyan-300 border-cyan-400/40 shadow-[0_0_10px_rgba(6,182,212,0.2)]';
                ...
                {selectedPlayer.achievements.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedPlayer.achievements.map((ach) => (
                      ...
                      >
                        {ach.achievement_type === 'BALLON_DOR' ? "Ballon d'Or" : ach.achievement_type === 'TOTS' ? 'TOTS' : ach.achievement_type.replace('_', ' ')}
                      </span>
                    ))}
                  </div>
                )}
