-- ==============================================================================
-- Migration: 20260921160000_unlimited_transfer_windows.sql
-- Description: Off-season unlimited transfer windows support
-- ==============================================================================

-- 1. Add is_unlimited column to transfer_windows
ALTER TABLE public.transfer_windows
ADD COLUMN IF NOT EXISTS is_unlimited BOOLEAN NOT NULL DEFAULT false;

-- 2. Helper function: get_team_transfer_slots
...
BEGIN
    ...
    SELECT id, name, is_open, start_date, end_date, is_unlimited
    INTO v_window
    ...
    -- Rule 1: Transfer dönemi açıkken
    IF FOUND THEN
        IF v_window.is_unlimited THEN
            -- Sınırsız transfer döneminde kota sınırsızdır
            RETURN QUERY SELECT 999, 0, 999, true, v_window.id, v_window.name;
        ELSE
            -- Normal transfer döneminde 5/5 gösterilir (dönem açıkken harcama sayacı düşmez)
    ...
    IF FOUND THEN
        IF v_window.is_unlimited THEN
            -- En son pencere sınırsız transfer penceresi ise slot kısıtlaması yoktur
            RETURN QUERY SELECT 999, 0, 999, false, v_window.id, v_window.name;
            RETURN;
        ELSE
...
-- 3. Update send_team_invite to enforce 5-slot quota ONLY when transfer window is NOT unlimited.
-- Retains row lock FOR UPDATE on league_teams and team_memberships to prevent concurrency race conditions.
...
BEGIN
    ...
    SELECT id, season_id, is_unlimited INTO v_window
    FROM public.transfer_windows
    WHERE is_open = true
      AND start_date <= CURRENT_TIMESTAMP
      AND end_date >= CURRENT_TIMESTAMP
    ...
    -- Enforce 5-slot quota per transfer window ONLY IF window is NOT unlimited
    IF NOT v_window.is_unlimited THEN
